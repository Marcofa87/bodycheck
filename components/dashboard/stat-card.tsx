        ...
        {dato ? (
          <>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs">
              ...
              <span className="text-muted-foreground">{formatter.data(dato.data)}</span>
            </div>
          </>
        ) : (
          <>
            <div className="text-3xl font-semibold tracking-tight text-muted-foreground/50">—</div>
            <p className="mt-2 text-xs text-muted-foreground">{t("dashboard.nessunDato")}</p>
          </>
        )}
      </CardContent>
    </Card>
...
function Variazione({
  ...
  if (delta === null) {
    return <span className="text-muted-foreground">{t("primaMisurazione")}</span>;
  }
  ...
  if (arrotondato === 0) {
    return (
      <span className="inline-flex items-center gap-0.5 rounded-md bg-muted px-1.5 py-0.5 font-medium text-muted-foreground">
        <MinusIcon className="size-3" /> {t("stabile")}
      </span>
    );
  }
  ...
  return (
    ...
    >
      ...
      {formatter.variazione(arrotondato, unita)}
    </span>
  );
}
